...
def broken_search(nums: List[int], target: int) -> int:
    ...
    while left < right:
        ...
        if nums[mid] == target:
            return mid
        # In a shifted list in order to choose which half to move to
        # we need to know the position of the shift. The shift can be
        # located based on the values of the left element (a),
        # the middle (m) and the target (t).
